...
description: Record a terminal session with remix/tui and manage input bytes, output buffers, dimensions, errors, and shutdown without the Node runner.
---

The album browser in [Terminal Applications](/terminal-applications/) leaves stdin, stdout, and terminal modes to `remix/tui/node`. Use `createRoot()` from `remix/tui` instead when your program already owns that I/O, or when there is no interactive terminal at all.

We'll reuse the finished `app.tsx` and `albums.ts` from that chapter, feed the app a short sequence of keys, and save its output to `albums.ansi`. No raw mode, alternate screen, or interactive stdin is needed to record the session.

## Record the album browser

Add `capture.tsx` beside the app in `demos/albums-tui`:

```tsx filename=demos/albums-tui/capture.tsx
import { writeFile } from "node:fs/promises";
import { createRoot } from "remix/tui";
...
```sh
cd demos/albums-tui
node --import remix/node-tsx capture.tsx
```
...
Each callback contains the changes from the previous frame. An unchanged tree can produce no output at all, and a selection change may update only a few cells. Preserve frame order and do not drop arbitrary frames when a transport falls behind. A later diff assumes the client applied the earlier ones.
...
`writeInput()` takes the bytes produced by a terminal, not a key name. For example:
...
The capture script needs none of these operations because it writes to a file. If your only target is an interactive Node terminal, use `remix/tui/node` rather than recreating its stream and signal handling.
...
The recording claims errors with `event.preventDefault()`, saves them, and unmounts the root. After cleanup, it throws the first error instead of writing a successful recording. An unclaimed error is rethrown asynchronously so the platform reports it. A terminal-specific failure is a `TerminalRenderError` whose `type` identifies the failure, while a component can throw any value.
...
The Node runner chooses that error policy for you: it treats root `error` events as fatal, restores the terminal, and rejects `closed`, even if another listener also calls `preventDefault()`. The zero-I/O root does not have a `closed` promise. Your transport decides when a session is finished and how errors reach its caller.

## Release the session

`unmount()` removes the component tree, aborts component signals, settles pending updates, and stops the root's input and animation timers. It is idempotent. The zero-I/O root leaves the last frame on the screen rather than clearing it or changing terminal modes.
...
The [root API](https://github.com/remix-run/remix/blob/main/packages/tui/src/lib/root.ts) documents its options and events, and the [Node runner](https://github.com/remix-run/remix/blob/main/packages/tui/src/lib/node.ts) shows terminal restoration around that API. To replace the terminal backend itself, continue with [Custom Renderers](/custom-renderers/).
