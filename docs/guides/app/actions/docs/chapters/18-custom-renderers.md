...
The terminal renderer, [`@pitlane/tui`](https://github.com/pitlane-tools/pitlane/tree/main/packages/tui), is this same interface with a real backend behind it. It lives in its own repository, where [Terminal applications](https://pitlane.tools/guides/terminal-applications) builds an interactive album list with `Box`, `Text`, `style()`, and `on()`, and [Embedding terminal renderers](https://pitlane.tools/guides/embedding-terminal-renderers) covers owning the bytes, resizing, and shutdown yourself.
