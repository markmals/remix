...
const background = rgba(20, 25, 32)
const foreground = rgba(232, 237, 242)
const muted = rgba(160, 177, 192)
const selectedBackground = rgba(37, 65, 88)
  ...
  return () => (
    ...
    >
      <Box mix={style({ layout: { width: grow(), direction: 'ttb' } })}>
        <Text mix={style({ color: accent })}>Remix / renderer lab</Text>
      ...
      >
        {items.length === 0 ? (
          <Text mix={style({ color: foreground })}>Queue empty. Press a to add a task.</Text>
        ) : (
...
function TaskRow(handle: Handle<{ item: QueueItem }>) {
  ...
  queue.terminal.addEventListener(
    ...
    (event) => {
      ...
      if (queue.selected === handle.props.item.id && (key.code === 'Enter' || key.code === ' ')) {
        increment()
      }
    },
