import { logHydrationMismatch } from '../client-entries.js';
import { patchHostProps } from '../core/props.js';
import { isFrameStartComment } from './frames.js';
import { getDocumentHead, isHeadType, isSvgParent, NO_PROPS } from './nodes.js';
/**
...
export const domHydration = {
    ...
    adoptElement(type, props, parent, cursor) {
        ...
        if (isHeadType(type)) {
            ...
            if (head !== null)
                return adoptHead(head, props, current, cursor);
        }
        ...
        if (currentTag === type)
            return adopt(current, props, cursor);
        ...
        if (candidate instanceof Element) {
            ...
            if (candidateTag === type)
                return adopt(candidate, props, cursor);
        }
...
function skipAdoptableComments(current, end) {
    ...
    while (node !== null && node !== end) {
        ...
        if (node.nodeType !== Node.COMMENT_NODE || isFrameStartComment(node))
            return node;
