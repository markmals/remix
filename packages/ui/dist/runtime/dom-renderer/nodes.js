 ...
 */
export function getOwnerDocument(parent) {
 ...
 */
export function isSvgParent(parent) {
 ...
 */
export function createHostElement(type, parent) {
    let doc = getOwnerDocument(parent);
    if (type === 'svg' || isSvgParent(parent))
        return doc.createElementNS(SVG_NS, type);
 ...
 */
export function getDocumentHead(parent) {
    if (parent instanceof Document)
        return parent.head;
    if (parent instanceof Node)
        return parent.ownerDocument?.head ?? null;
    return null;
}
