...
const NO_DESCRIPTORS: OnMixinDescriptor[] = []
 ...
 * mixin runtime.
 */
export function resolveDirectEventDescriptors(
  mix: MixinRuntimeValue | undefined,
  ...
  return mix as OnMixinDescriptor[]
}
 ...
 */
export function syncDirectEventListeners(
  target: EventTarget,
  descriptors: OnMixinDescriptor[] | undefined,
  state: DirectEventState | undefined,
): DirectEventState | undefined {
 ...
 */
export function teardownDirectEventListeners(
  target: EventTarget,
  state: DirectEventState | undefined,
): void {
  if (!state) return
 ...
 */
export function abandonDirectEventListeners(state: DirectEventState | undefined): void {
  if (!state) return
  let bindings = state.bindings
  for (let index = 0; index < bindings.length; index++) {
...
function attach(target: EventTarget, binding: DirectEventBinding): void {
  let stableHandler = binding.stableHandler
  if (!stableHandler) {
    stableHandler = (event: Event) => {
...
function detach(target: EventTarget, binding: DirectEventBinding): void {
...
function abortReentry(binding: DirectEventBinding, reason: string): void {
