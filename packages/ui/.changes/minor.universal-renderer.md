...
Add the experimental `remix/ui/renderer` host-operation API, shared by the DOM renderer and out-of-tree backends such as the terminal renderer in [`@pitlane/tui`](https://github.com/pitlane-tools/pitlane/tree/main/packages/tui). Hosts provide node creation, composed-prop updates, insertion/removal, traversal, and optional commit and event-target hooks. One reconciliation engine manages component and mixin lifecycles, keyed identity, context, batched updates, post-commit tasks, and abort-signal cleanup.
