 ...
 * against it. Listen for it with the `on` mixin from `remix/ui`.
 */
 ...
 * with `on` from `remix/ui` sees {@link TerminalPointerEvent} instances
    ...
    createElement(type, props) {
      ...
      if (type === BOX) {
        ...
        throw new TerminalRenderError(
          'UNSUPPORTED_ELEMENT',
          `<${type}> is not a terminal element. Render Box and Text from @remix-run/tui instead.`,
        )
      }
