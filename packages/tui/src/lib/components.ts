...
import { BOX, TEXT, type TerminalBox, type TerminalTextElement } from './host.ts'
...
export interface BoxProps {
  ...
  /**
   * Mixins applied to the box: `style` from `remix/tui` for layout and
   * paint, and `on` from `remix/ui` for `pointerenter`, `pointerleave`,
   * and `pointerclick`.
   */
...
export interface TextProps {
  /**
   * Mixins applied to the text run. `style` from `remix/tui` sets color,
 ...
 * @example
 * ```tsx
 * import { style, Text } from 'remix/tui'
